...
import com.blockelot.rpg.RpgPlayer.CustomEvents.ArmorEquip.ArmorEquipEvent.EquipMethod;
...
public class ArmorListener implements Listener {
    ...
    public Inventory getClickedInventory(final InventoryClickEvent event) {
    ...
    public final void onInventoryClick(final InventoryClickEvent e) {
        ...
        if (getClickedInventory(e) != null && !getClickedInventory(e).getType().equals(InventoryType.PLAYER)) {
            return;
        }
        ...
        if (shift) {
            ...
            if (numberkey) {
                ...
                if (getClickedInventory(e).getType().equals(InventoryType.PLAYER)) {// Prevents shit in the 2by2 crafting
                    // e.getClickedInventory() == The players inventory
                    // e.getHotBarButton() == key people are pressing to equip or unequip the item to or from.
                    // e.getRawSlot() == The slot the item is going to.
                    // e.getSlot() == Armor slot, can't use e.getRawSlot() as that gives a hotbar slot ;-;
                    ItemStack hotbarItem = getClickedInventory(e).getItem(e.getHotbarButton());
                    if (hotbarItem != null) {// Equipping
                        newArmorType = ArmorType.matchType(hotbarItem);
                        newArmorPiece = hotbarItem;
                        oldArmorPiece = getClickedInventory(e).getItem(e.getSlot());
                    } else {// Unequipping
    ...
    public void playerInteractEvent(PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (e.getClickedBlock() != null && e.getAction() == Action.RIGHT_CLICK_BLOCK) {// Having both of these checks is useless, might as well do it though.
                ...
                for (String s : ArmorBlockedMaterials.GetList()) {
